...
type CasingType = 'snake_case' | 'camelCase' | undefined;
// Simplified logging for API usage
const withStyle = { errorWarning: (msg: string) => console.warn(msg) };
// Simplified types for API usage
type IntrospectStage = string;
type IntrospectStatus = string;
import type {
	CheckConstraint,
	Column,
	ForeignKey,
...
function filterIgnoredTablesByField(fieldName: string) {
	...
	return `${fieldName} != '__drizzle_migrations'
			AND ${fieldName} NOT LIKE '\\_actor\\_%' ESCAPE '\\'
