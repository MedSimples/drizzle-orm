...
// src/durable-server.ts
import { DurableObject } from "cloudflare:workers";
var DurableObjectRpc = class extends RpcTarget4 {
  static {
    __name(this, "DurableObjectRpc");
  }
  ctx;
  env;
  constructor(ctx, env) {
  ...
  getValue(keys, options) {
    return this.ctx.storage.get(keys, options);
  }
  list(options) {
    return this.ctx.storage.list(options);
  }
  putValue(key, value, options) {
    return this.ctx.storage.put(key, value, options);
  }
  deleteValue(keys, options) {
    return this.ctx.storage.delete(keys, options);
  }
  deleteAll(options) {
    return this.ctx.storage.deleteAll(options);
  }
  transaction(closure) {
    return this.ctx.storage.transaction(closure);
  }
  getAlarm(options) {
    return this.ctx.storage.getAlarm(options);
  }
  setAlarm(scheduledTime, options) {
    return this.ctx.storage.setAlarm(scheduledTime, options);
  }
  deleteAlarm(options) {
    return this.ctx.storage.deleteAlarm(options);
  }
  sync() {
  ...
  waitForBookmark(bookmark) {
    return this.ctx.storage.waitForBookmark(bookmark);
  }
  alarm(scheduledTime, options) {
    return this.ctx.storage.setAlarm(scheduledTime, options);
  }
  databaseSize() {
...
var DurableServer = class extends DurableObject {
  static {
    __name(this, "DurableServer");
  }
  identifier;
  constructor(ctx, env) {
    super(ctx, env);
    this.ctx.storage.sql.exec(`
		...
		`);
    this.identifier = this.ctx.storage.sql.exec("SELECT identifier FROM _metadata limit 1")[0]?.identifier;
  }
  fetch(request) {
    try {
      if (!this.identifier) {
        ...
        return this.ctx.storage.transaction(async () => {
          ...
          try { 
            const response = await newWorkersRpcResponse(
              request, 
              new DurableObjectRpc(this.ctx, this.env),
              {
                onSendError(error) {
                  reject(error);
                }
  ...
  setMetaData(doIdentifier) {
    if (this.identifier) {
      return;
    }
    if (!doIdentifier) {
      throw new Error("doIdentifier is required");
    }
    this.ctx.storage.sql.exec("INSERT INTO _metadata (identifier) VALUES (?) ON CONFLICT DO NOTHING", doIdentifier);
    this.identifier = doIdentifier;
  }
};

// src/worker.ts
var Entrypoint = class extends WorkerEntrypoint {
  static {
    __name(this, "Entrypoint");
  }
  async fetch(req) {
    const { searchParams } = new URL(req.url);
    const id = searchParams.get("id");
    const ns = this.ctx?.exports?.DurableServer;
    const name = id ?? 'default';
    const stub = ns.getByName(name);
    await stub.setMetaData(name);
    return stub.fetch(req);
  }
};
var worker_default = Entrypoint;
export {
  DurableServer,
